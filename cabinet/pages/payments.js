(function() {
    ...
    function openPackageModal(packageId) {
        ...
        if (pkg) {
            ...
            if (nameEl) nameEl.textContent = `Купівля пакету "${pkg.name}"`;
            if (descEl) descEl.textContent = `Вартість: ${pkg.price} грн`;
            if (priceEl) priceEl.innerHTML = `<strong>${pkg.price} грн</strong> буде списано з вашого рахунку`;
    ...
    function openTopupModal() { updateCardSelect(); const modal = document.getElementById('topupModal'); if (modal) modal.style.display = 'flex'; }
    ...
    function updateCardSelect() {
        ...
        if (select && savedCards.length > 0) {
            ...
            if (cardSelect) cardSelect.style.display = 'block';
        } else { 
            const cardSelect = document.getElementById('cardSelect');
            if (cardSelect) cardSelect.style.display = 'none';
        }
    }
