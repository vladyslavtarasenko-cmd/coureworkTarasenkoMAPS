...
function updateToRateCount() { const el = document.getElementById('toRateCount'); if (el) el.textContent = toRateList.length; }
...
function renderMyReviews() {
    ...
    if (!container) return;
    if (!receivedRatings.length) { container.innerHTML = '<div class="empty-state">📝 Поки немає відгуків про вас</div>'; return; }
...
function renderGivenReviews(filter = 'all') {
    ...
    if (!filtered.length) { container.innerHTML = '<div class="empty-state">⭐ Ви ще не залишили жодної оцінки</div>'; return; }
...
function renderReceivedReviews(filter = 'all') {
    ...
    if (filter !== 'all') filtered = filtered.filter(r => r.rating === parseInt(filter));
    if (!filtered.length) { container.innerHTML = '<div class="empty-state">📊 Поки немає відгуків</div>'; return; }
...
function renderToRateList() {
    ...
    if (!container) return;
    if (!toRateList.length) { container.innerHTML = '<div class="empty-state">✅ У вас немає продавців на оцінку</div>'; return; }
...
function openRateModal(itemId) {
    ...
    if (currentRateItem) {
        ...
        document.getElementById('rateProductInfo').innerHTML = `<strong>${currentRateItem.product}</strong><br>Ціна: ${currentRateItem.price} грн<br>Дата: ${currentRateItem.date}`;
        selectedRating = 0;
        updateStars(0);
...
function filterGivenReviews() { renderGivenReviews(document.getElementById('givenFilter').value); }
function filterReceivedReviews() { renderReceivedReviews(document.getElementById('receivedFilter').value); }
function likeReview(id) { const r = [...receivedRatings, ...givenRatings].find(r => r.id === id); if(r){ r.likes = (r.likes||0)+1; saveReceivedRatings(); saveGivenRatings(); renderAll(); } }
