...
// Характеристики для різних категорій
const categorySpecs = {
    electronics: [
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Заповнення місцезнаходження
    const locationInput = document.getElementById('adLocation');
    if (locationInput) {
        locationInput.value = localStorage.getItem('user_location') || 'Київ';
    }
    
    // Заповнення контактної особи
    const contactInput = document.getElementById('adContact');
    if (contactInput && currentUser) {
        contactInput.value = currentUser.name || '';
    }
});
            ...
            if (spec.type === 'select') {
                return `
                    <div class="spec-row">
                        ...
                        <select name="${spec.name}" class="spec-input">
                ...
                return `
                    <div class="spec-row">
                        <label>${spec.label}</label>
                        <input type="${spec.type}" name="${spec.name}" placeholder="${spec.placeholder || ''}" class="spec-input">
                    </div>
                `;
            }
        }).join('');
...
function handlePhotos(input) {
    ...
    files.forEach(file => {
        if (uploadedPhotos.length >= 8) {
            alert('Можна додати максимум 8 фото');
            return;
        }
...
function submitAd(event) {
    ...
    const price = document.getElementById('priceFree').checked ? 0 : parseFloat(document.getElementById('adPrice').value);
    ...
    if (!title || title.length < 16) {
        alert('Назва повинна містити щонайменше 16 символів');
        return;
    }
    ...
    if (!price && price !== 0 && !isFree) {
        alert('Вкажіть ціну');
        return;
    }
    
    if (!description || description.length < 40) {
        alert('Опис повинен містити щонайменше 40 символів');
        return;
    }
    
    if (!location) {
        alert('Вкажіть місцезнаходження');
        return;
    }
...
// Додаємо обробник ціни при завантаженні
setupPriceLogic();
